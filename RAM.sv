...
module RAM  
  ...
  integer i;  
  reg  [7:0] ram [0:31]; 			//1K nos of 8-bit memory reg
  wire [9:0] ram_addr; 				//10-bit internal address for 1K memory
  assign ram_addr = addr[9:0];  	//mapping global 32-bit addr to internal 10-bit address
...
*/
  ...
  always @(posedge clk)
  begin
	 if (CS && !RW_ && (ram_addr <= 31)) begin
      ram[ram_addr] <= data_in[31:24];     	//write MSB to lower address
      ram[ram_addr+1] <= data_in[23:16];  			//write LSB to higher address
...
//	// Read transaction	 
	always @(posedge clk)
	begin
	  if (CS && RW_ && (ram_addr <= 31)) 
	    data_out = {ram[ram_addr], ram[ram_addr+1], ram[ram_addr+2], ram[ram_addr+3]};
	  else
	    data_out = 32'bz;
	end	
